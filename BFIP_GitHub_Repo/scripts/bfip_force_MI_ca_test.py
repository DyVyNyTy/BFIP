...
def simulate_force_mi_ca(T0=300.0):
...
def plot_force_mi_ca(t, amps, thetas, dGs, bfips):
    plt.figure(figsize=(12, 10))

    plt.subplot(4, 1, 1)
    plt.plot(t, amps, 'k--')
    plt.ylabel('Amplitude')
    plt.title('Forced MI > 2.2 — BFIP Flicker Test (Ca²⁺)')
    ...
    plt.tight_layout()
    plt.savefig('BFIP_force_MI_ca_test.png', dpi=600)
    plt.show()

if __name__ == '__main__':
    t, amps, thetas, dGs, bfips = simulate_force_mi_ca()
    plot_force_mi_ca(t, amps, thetas, dGs, bfips)
