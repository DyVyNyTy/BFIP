...
def map_phase(dH_range=(-100, -30), dS_range=(-0.25, 0.00), step=2.5, amp=1.0):
    dH_vals = np.arange(dH_range[0], dH_range[1] + step, step)
    dS_vals = np.arange(dS_range[0], dS_range[1] + step, step)
...
def plot_map(dH_vals, dS_vals, Z):
    plt.figure(figsize=(10, 6))
    plt.imshow(Z.T, origin='lower', aspect='auto',
               extent=[dH_vals[0], dH_vals[-1], dS_vals[0], dS_vals[-1]],
               cmap='coolwarm', interpolation='nearest')
    plt.colorbar(label='BFIP Activation (1=ON, 0=OFF)')
    plt.xlabel('ΔH (kJ/mol)')
    plt.ylabel('ΔS (kJ/mol·K)')
    plt.title('Fe²⁺ BFIP Activation Map (ΔH–ΔS, Amp = 1.0)')
    plt.tight_layout()
    plt.savefig('Fe2_BFIP_activation_map.png', dpi=600)
    plt.show()

if __name__ == '__main__':
    dH_vals, dS_vals, Z = map_phase()
    plot_map(dH_vals, dS_vals, Z)
