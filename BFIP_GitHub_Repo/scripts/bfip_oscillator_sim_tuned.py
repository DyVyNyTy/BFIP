...
def simulate_oscillator(T0=300.0):
    ...
    dH = -40     # softened binding enthalpy
    dS = -0.05   # softened entropy contribution
...
def plot_oscillator(t, amps, bfip):
    ...
    plt.title('BFIP Oscillator Simulation (ΔH/ΔS Tuned for Flicker)')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('BFIP_oscillator_sim_tuned.png', dpi=600)
    plt.show()
