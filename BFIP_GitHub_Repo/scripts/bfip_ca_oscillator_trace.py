...
def simulate_ca_trace(T0=300.0):
...
def plot_ca_trace(t, amps, thetas, mis, dGs, bfips):
    plt.figure(figsize=(12, 10))

    plt.subplot(4, 1, 1)
    plt.plot(t, amps, 'k--')
    plt.ylabel('Amplitude')
    plt.title('Ca²⁺ BFIP Oscillator Diagnostic Trace')
    ...
    plt.tight_layout()
    plt.savefig('BFIP_ca_oscillator_trace.png', dpi=600)
    plt.show()

if __name__ == '__main__':
    t, amps, thetas, mis, dGs, bfips = simulate_ca_trace()
    plot_ca_trace(t, amps, thetas, mis, dGs, bfips)
