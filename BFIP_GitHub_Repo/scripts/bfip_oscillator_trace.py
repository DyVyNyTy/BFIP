...
def simulate_trace(T0=300.0):
    ...
    return t_span, np.array(amplitudes), np.array(thetas), np.array(mis), np.array(deltaGs), np.array(bfip_states)

def plot_trace(t, amps, thetas, mis, bfip):
    plt.figure(figsize=(12, 10))

    plt.subplot(4, 1, 1)
    plt.plot(t, amps, 'k--')
    plt.ylabel('Amplitude')
    plt.title('BFIP Oscillator Diagnostic Trace')
    ...
    plt.subplot(4, 1, 4)
    plt.step(t, bfip, where='mid', color='purple')
    plt.ylabel('BFIP State')
    plt.xlabel('Time (s)')

    plt.tight_layout()
    plt.savefig('BFIP_oscillator_trace.png', dpi=600)
    plt.show()

if __name__ == '__main__':
    t, amps, thetas, mis, dgs, bfips = simulate_trace()
    plot_trace(t, amps, thetas, mis, bfips)
