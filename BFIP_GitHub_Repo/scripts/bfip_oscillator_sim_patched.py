...
def sinusoidal_amp(t, base=0.35, amp=0.35, freq=1/60):
...
def plot_oscillator(t, amps, bfip):
    ...
    plt.title('BFIP Oscillator Simulation (H⁺ Clock, Toggling Enabled)')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('BFIP_oscillator_sim_patched.png', dpi=600)
    plt.show()
